    ...
    public static string Compile(JsonElement template, JsonElement? labelData, JsonElement? configuration, int dpmm, JsonElement? project = null)
    {
        ...
        foreach (var field in CollectFields(template))
        {
            ...
            if (type == "machineschecklist" || key == "machines")
            {
                ...
                // Same split as Opti HTML: "CUT>POL>WJT" → one row + tick box per machine.
                var items = raw.Split('>', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
                if (items.Length == 0) continue;
                var rowH = Math.Max(14, fh / items.Length);
                var fontH = Math.Max(10, Math.Min(rowH - 2, MapS(Num(Get(field, "fontSize"), 11))));
                var box = Math.Max(8, Math.Min(fontH, (int)Math.Round(fontH * 0.85)));
                var gap = 4;
                var tw = Math.Max(8, fw - box - gap);
                for (var i = 0; i < items.Length; i++)
                {
                    var yy = fy + i * rowH;
                    var gfa = GfaEncoder.TextField(ZplUtil.Sanitize(items[i]), tw, rowH, fontH, "Arial", FontStyle.Regular, StringAlignment.Near, false);
                    sb.AppendLine($"^FO{fx},{yy}{gfa}^FS");
                    var boxY = yy + Math.Max(0, (rowH - box) / 2);
                    sb.AppendLine(Inv("^FO{0},{1}^GB{2},{3},2^FS", fx + tw + gap, boxY, box, box));
                }
                continue;
            }
            ...
            var gfaText = GfaEncoder.TextField(text, fw, fh, fontH2, family, style, align, inverted);
